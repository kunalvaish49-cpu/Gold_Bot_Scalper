...
double historyAvgMomentum   = 0.0;   // avg signed momentum (priceChange) across history buffer
double historyAvgPressure   = 0.0;   // avg net pressure (buyerPressure - sellerPressure)
double historyAvgVolatility = 0.0;   // avg candle aggressiveness (wick exposure) across history buffer
double historyAvgStability  = 0.0;   // avg candle stability (wick symmetry) across history buffer
double historyAvgConfidence = 0.0;   // avg candle confidence across history buffer
double historyAvgDirection  = 0.0;   // avg direction_M1 (-1..1), i.e. history trend bias

// Auto-cut state (shared between AutoCut module and Minds 2/4)
datetime autoExitTime      = 0;
bool     autoExitActive    = false;
string   autoExitMind      = "";
datetime autoExitStartTime = 0;   // when the auto-exit sequence was armed, used for elapsed-time logging

// Detector stability (state confirmation / hysteresis)
MARKET_STATE candidateState      = STATE_NEUTRAL_CHOPPY;
int          candidateStateCount = 0;
...
input int    StateConfirmationBars   = 3;     // Consecutive winning ticks required before a state change is confirmed
...
int OnInit()
{
    ...
    historyAvgSpeed      = 0.0;
    historyAvgRange      = 0.0;
    historyAvgMomentum   = 0.0;
    historyAvgPressure   = 0.0;
    historyAvgVolatility = 0.0;
    historyAvgStability  = 0.0;
    historyAvgConfidence = 0.0;
    historyAvgDirection  = 0.0;
    ...
    candidateState      = currentMarketState;
...
void InputManager()
{
    ...
    if(ClassificationThreshold < 0.0 || ClassificationThreshold > 1.0)
        Logger(LOG_WARNING, "ClassificationThreshold should be between 0.0 and 1.0. Current: " +
               DoubleToString(ClassificationThreshold, 2));
    if(ConfidenceThreshold < 0.0 || ConfidenceThreshold > 100.0)
        Logger(LOG_WARNING, "ConfidenceThreshold should be between 0.0 and 100.0. Current: " +
               DoubleToString(ConfidenceThreshold, 2));
    if(MagicNumber <= 0)
        Logger(LOG_WARNING, "MagicNumber must be > 0. Current: " + IntegerToString(MagicNumber));
    if(Mind2_EnableAutoCut && Mind2_AutoCutWait <= 0)
        Logger(LOG_WARNING, "Mind2_AutoCutWait must be > 0 when Mind2_EnableAutoCut is true. Current: " +
               IntegerToString(Mind2_AutoCutWait));
    if(Mind4_EnableAutoCut && Mind4_AutoCutWait <= 0)
        Logger(LOG_WARNING, "Mind4_AutoCutWait must be > 0 when Mind4_EnableAutoCut is true. Current: " +
               IntegerToString(Mind4_AutoCutWait));
    if(StateConfirmationBars < 1)
        Logger(LOG_WARNING, "StateConfirmationBars must be >= 1. Current: " +
               IntegerToString(StateConfirmationBars));
}
...
void MarketMemory_Update()
{
    ...
    // Single pass over the buffer accumulates every history-derived metric to avoid repeated loops.
    if(historyCount > 0)
    {
        double sumSpeed = 0.0, sumRange = 0.0;
        double sumMomentum = 0.0, sumPressure = 0.0, sumVolatility = 0.0;
        double sumStability = 0.0, sumConfidence = 0.0, sumDirection = 0.0;

        for(int i = 0; i < historyCount; i++)
        {
            int idx = (historyHead - 1 - i + historySize) % historySize;
            MarketObservation hObs = observationHistory[idx];

            sumSpeed      += hObs.speed;
            sumRange      += hObs.range_M1;
            sumMomentum   += hObs.momentum;
            sumPressure   += (hObs.buyerPressure - hObs.sellerPressure);
            sumVolatility += hObs.candleAggressiveness;
            sumStability  += hObs.candleStability;
            sumConfidence += hObs.candleConfidence;
            sumDirection  += (double)hObs.direction_M1;
        }

        historyAvgSpeed      = sumSpeed      / historyCount;
        historyAvgRange      = sumRange      / historyCount;
        historyAvgMomentum   = sumMomentum   / historyCount;
        historyAvgPressure   = sumPressure   / historyCount;
        historyAvgVolatility = sumVolatility / historyCount;
        historyAvgStability  = sumStability  / historyCount;
        historyAvgConfidence = sumConfidence / historyCount;
        historyAvgDirection  = sumDirection  / historyCount;
    }
                             ...
                             bool memoryReady)
{
    ...
    // Raw winner: state change only if winning score clears the classification threshold
    MARKET_STATE rawState = currentMarketState;
    string       rawReason = "no score cleared ClassificationThreshold";
    if(maxScore >= ClassificationThreshold)
    {
        if     (scoreUp   > scoreDown && scoreUp   > scoreChop && scoreUp   > scoreVol) { rawState = STATE_STRONG_UPTREND;   rawReason = "Uptrend score highest";   }
        else if(scoreDown > scoreUp   && scoreDown > scoreChop && scoreDown > scoreVol) { rawState = STATE_STRONG_DOWNTREND; rawReason = "Downtrend score highest"; }
        else if(scoreVol  > scoreUp   && scoreVol  > scoreChop && scoreVol  > scoreDown) { rawState = STATE_HIGH_VOLATILITY;  rawReason = "Volatility score highest"; }
        else if(scoreChop > scoreUp   && scoreChop > scoreDown && scoreChop > scoreVol) { rawState = STATE_NEUTRAL_CHOPPY;   rawReason = "Choppy score highest";    }
        // Ties: keep current state (no change)
    }

    // Detector Stability: require StateConfirmationBars consecutive ticks agreeing on the
    // same new state before actually switching currentMarketState. This uses only the
    // existing Observation History (via the scoring above) to reduce rapid oscillation -
    // no new market states are introduced.
    MARKET_STATE newState = currentMarketState;
    if(rawState == currentMarketState)
    {
        // Already agrees with current state - no candidate pending
        candidateState      = currentMarketState;
        candidateStateCount = 0;
    }
    else
    {
        ...
        if(candidateStateCount >= StateConfirmationBars)
        {
            newState             = candidateState;
            candidateStateCount  = 0;
        }
        // else: not yet confirmed, currentMarketState is held
    }
    ...
    if(stateChanged)
    {
        ...
        Logger(LOG_DETECTOR,
               "STATE CHANGE DETAIL | PrevState: " + EnumToString(currentMarketState) +
               " | NewState: "            + EnumToString(newState) +
               " | DetectorConfidence: "  + DoubleToString(detectorConfidence, 1) + "%" +
               " | ObservationConfidence: " + DoubleToString(currentAnalysis.observationConfidence * 100.0, 1) + "%" +
               " | RunningCandle[Dir:"    + IntegerToString(currentObservation.direction_M1) +
               " Body%:"                  + DoubleToString(currentObservation.bodyPct_M1, 1) +
               " Quality:"                + DoubleToString(currentObservation.candleQuality, 2) + "]" +
               " | 35CandleSummary[NetDir:" + IntegerToString(currentAnalysis.netDirection) +
               " DirStr:"                 + DoubleToString(currentAnalysis.directionStrength, 2) +
               " CandleQ:"                + DoubleToString(currentAnalysis.candleQuality, 2) +
               " TrendQ:"                 + DoubleToString(currentAnalysis.trendQuality, 2) + "]" +
               " | Reason: " + rawReason + " (confirmed after " + IntegerToString(StateConfirmationBars) + " ticks)");
    }
    else
        Logger(LOG_DETECTOR,
               ...
               " Ch="   + DoubleToString(scoreChop, 3) +
               " Vo="   + DoubleToString(scoreVol, 3)  +
               " | Candidate: " + EnumToString(candidateState) +
               " (" + IntegerToString(candidateStateCount) + "/" + IntegerToString(StateConfirmationBars) + ")");
...
void TradeManager_CloseByComment(string commentFilter, string autoCutReason = "", int autoCutElapsedSec = -1, string autoCutMind = "")
{
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if(positionInfo.SelectByIndex(i))
               ...
               StringFind(positionInfo.Comment(), commentFilter) >= 0)
            {
                ...
                // AutoCut callers pass a reason so the closure is logged under LOG_AUTOCUT
                // with full context (ticket, reason, elapsed time, mind). Otherwise, a plain
                // manual/other closure is logged under LOG_TRADE as before.
                if(autoCutReason != "")
                    Logger(LOG_AUTOCUT, "Ticket: " + IntegerToString((int)ticket) +
                           " | Reason: "  + autoCutReason +
                           " | Elapsed: " + IntegerToString(autoCutElapsedSec) + "s" +
                           " | Mind: "    + autoCutMind);
                else
...
void AutoCut()
{
    ...
    {
        int elapsedSec = (int)(TimeCurrent() - autoExitStartTime);
        TradeManager_CloseByComment(autoExitMind, "Timed auto-exit", elapsedSec, autoExitMind);
        Logger(LOG_AUTOCUT, "AutoCut sequence complete | Mind: " + autoExitMind +
               " | Elapsed: " + IntegerToString(elapsedSec) + "s");
        autoExitActive    = false;
        autoExitMind       = "";
        autoExitStartTime = 0;
    }
...
//+------------------------------------------------------------------+
// HELPER: DollarToPoints
//+------------------------------------------------------------------+
// Converts a dollar amount into a number of points for TradingSymbol at
// the given lot size, using live broker symbol properties only.
// No hardcoded pip divisors, no assumptions about Gold's tick size.
//
//   valuePerPoint = (SYMBOL_TRADE_TICK_VALUE / SYMBOL_TRADE_TICK_SIZE) * SYMBOL_POINT * lots
//   points        = dollarAmount / valuePerPoint
//
// Callers multiply the returned points by SYMBOL_POINT to get a price distance,
// exactly like the previous PipDivisor-based calculation did.

double DollarToPoints(double dollarAmount, double lots)
{
    ...
    if(tickValue <= 0.0 || tickSize <= 0.0 || point <= 0.0 || lots <= 0.0)
    {
        Logger(LOG_ERROR, "DollarToPoints: invalid symbol data (tickValue/tickSize/point/lots) - returning 0");
        return 0.0;
    }

    double valuePerPoint = (tickValue / tickSize) * point * lots;
    if(valuePerPoint <= 0.0)
    {
        Logger(LOG_ERROR, "DollarToPoints: computed valuePerPoint <= 0 - returning 0");
        return 0.0;
    }
...
//+------------------------------------------------------------------+
// HELPER: Logger_SkippedTrade
//+------------------------------------------------------------------+
// Called by a Mind when its gating placeholder function returns false,
// so a trade is not opened. Keeps skipped-trade logging consistent
// across all four Minds.

void Logger_SkippedTrade(int mindNumber, string placeholderName)
{
    Logger(LOG_WARNING, "Mind" + IntegerToString(mindNumber) +
           " skipped | Placeholder: " + placeholderName +
           " | Reason: Strategy Not Yet Defined");
}
...
void Mind1()
{
    ...
    // Gate: only trade if the Buy Opportunity placeholder confirms a signal.
    // Strategy logic itself remains untouched - this only prevents accidental trades
    // while FindBuyOpportunity() is still a TODO.
    if(!FindBuyOpportunity())
    {
        Logger_SkippedTrade(1, "FindBuyOpportunity");
        return;
    }

    double ask   = SymbolInfoDouble(TradingSymbol, SYMBOL_ASK);
    double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
    double tp    = ask + DollarToPoints(Mind1_TP, LotSize) * point;
    double sl    = ask - DollarToPoints(Mind1_SL, LotSize) * point;
...
void Mind2()
{
    ...
    // Gate: Mind2's Buy/Sell opportunity logic is not yet defined.
    // This placeholder decision must stay false until that strategy is written,
    // to prevent Mind2 from auto-buying on every Neutral/Choppy tick.
    bool mind2Signal = false; // TODO: define Mind2 Buy/Sell Opportunity condition before executing
    if(!mind2Signal)
    {
        Logger_SkippedTrade(2, "Mind2_Signal");
        return;
    }

    double ask   = SymbolInfoDouble(TradingSymbol, SYMBOL_ASK);
    double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
    double tp    = ask + DollarToPoints(Mind2_TP, LotSize) * point;
    double sl    = ask - DollarToPoints(Mind2_SL, LotSize) * point;
...
void Mind3()
{
    ...
    // Gate: only trade if the Sell Opportunity placeholder confirms a signal.
    if(!FindSellOpportunity())
    {
        Logger_SkippedTrade(3, "FindSellOpportunity");
        return;
    }

    double bid   = SymbolInfoDouble(TradingSymbol, SYMBOL_BID);
    double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
    double tp    = bid - DollarToPoints(Mind3_TP, LotSize) * point;
    double sl    = bid + DollarToPoints(Mind3_SL, LotSize) * point;
...
void Mind4()
{
    ...
    // Gate: only trade if the Opposite Trade placeholder confirms a signal.
    if(!FindOppositeTrade())
    {
        Logger_SkippedTrade(4, "FindOppositeTrade");
        return;
    }

    double bid   = SymbolInfoDouble(TradingSymbol, SYMBOL_BID);
    double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
    double tp    = bid - DollarToPoints(Mind4_TP, LotSize) * point;
    double sl    = bid + DollarToPoints(Mind4_SL, LotSize) * point;
