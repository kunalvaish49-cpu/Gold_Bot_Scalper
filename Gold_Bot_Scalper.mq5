...
MARKET_STATE previousMarketState = STATE_NEUTRAL_CHOPPY;
...
double historyAvgMomentum   = 0.0;
double historyAvgPressure   = 0.0;
double historyAvgVolatility = 0.0;
double historyAvgStability  = 0.0;
double historyAvgConfidence = 0.0;
int    historyNetDirection  = 0;      // running tally of direction

// State confirmation buffer (for detector stability)
int stateConfirmationBars = 0;

// Auto-cut state (shared between AutoCut module and Minds 2/4)
datetime autoExitTime   = 0;
bool     autoExitActive = false;
string   autoExitMind   = "";
datetime autoExitStart  = 0;
...
input int    StateConfirmationBars   = 1;    // Number of consecutive bars to confirm state
...
input double PipDivisor     = 10.0;    // Legacy - kept for input validation; use DollarToPoints() for TP/SL
input int    LogIntervalSec = 60;      // Periodic status log interval (seconds)

//+------------------------------------------------------------------+
// HELPER FUNCTION: DollarToPoints
//+------------------------------------------------------------------+
// Converts dollar TP/SL value to price distance (points) using broker symbol properties.
// Calculation: dollarAmount / (tickValue * lotSize) = points
// This ensures TP and SL are exact dollar amounts, not pip distances.

double DollarToPoints(double dollarAmount, double lotSize)
{
    ...
    {
        Logger(LOG_ERROR, "DollarToPoints: Invalid symbol properties | TickValue: " +
               DoubleToString(tickValue, 5) + " | TickSize: " + DoubleToString(tickSize, 5));
        return 0.0;
    }

    // Calculate how many points = 1 tick
    double pointsPerTick = tickSize / point;

    // Calculate how many ticks for the dollar amount at this lot size
    double requiredTicks = dollarAmount / (tickValue * lotSize);

    // Convert ticks to points
    double points = requiredTicks * pointsPerTick;
...
int OnInit()
{
    ...
    stateConfirmationBars = 0;

    // Initialize history averages
    historyAvgSpeed      = 0.0;
    historyAvgRange      = 0.0;
    ...
    Logger(LOG_INFO, "EA initialized | Symbol: " + TradingSymbol +
           ...
           " | ConfirmBars: "      + IntegerToString(StateConfirmationBars) +
           " | DollarToPoints OK");
...
void InputManager()
{
    ...
    // Market Detector validation
    if(RunningCandleWeight < 0.0)
    {
        Logger(LOG_WARNING, "RunningCandleWeight must be >= 0. Current: " + DoubleToString(RunningCandleWeight, 2));
        validationPassed = false;
    }

    if(ClosedCandleWeight < 0.0)
    {
        Logger(LOG_WARNING, "ClosedCandleWeight must be >= 0. Current: " + DoubleToString(ClosedCandleWeight, 2));
        validationPassed = false;
    }

    if(RunningCandleWeight + ClosedCandleWeight <= 0.0)
    {
        Logger(LOG_WARNING, "RunningCandleWeight + ClosedCandleWeight must sum to > 0.");
        validationPassed = false;
    }

    if(ClassificationThreshold < 0.0 || ClassificationThreshold > 1.0)
    {
        Logger(LOG_WARNING, "ClassificationThreshold must be 0.0 to 1.0. Current: " + DoubleToString(ClassificationThreshold, 2));
        validationPassed = false;
    }

    if(ConfidenceThreshold < 0.0 || ConfidenceThreshold > 100.0)
    {
        Logger(LOG_WARNING, "ConfidenceThreshold must be 0.0 to 100.0. Current: " + DoubleToString(ConfidenceThreshold, 2));
        validationPassed = false;
    }

    if(StateConfirmationBars < 1)
    {
        Logger(LOG_WARNING, "StateConfirmationBars must be >= 1. Current: " + IntegerToString(StateConfirmationBars));
        validationPassed = false;
    }

    // Trade settings validation
    if(LotSize <= 0.0)
    {
    ...
    if(Mind1_TP <= 0.0)
    {
        Logger(LOG_WARNING, "Mind1_TP must be > 0. Current: " + DoubleToString(Mind1_TP, 2));
        validationPassed = false;
    }

    if(Mind1_SL <= 0.0)
    {
        Logger(LOG_WARNING, "Mind1_SL must be > 0. Current: " + DoubleToString(Mind1_SL, 2));
        validationPassed = false;
    }

    if(Mind1_MaxConcurrent < 1)
    {
        Logger(LOG_WARNING, "Mind1_MaxConcurrent must be >= 1. Current: " + IntegerToString(Mind1_MaxConcurrent));
        validationPassed = false;
    }
    ...
    if(Mind2_TP <= 0.0)
    {
        Logger(LOG_WARNING, "Mind2_TP must be > 0. Current: " + DoubleToString(Mind2_TP, 2));
        validationPassed = false;
    }

    if(Mind2_SL <= 0.0)
    {
        Logger(LOG_WARNING, "Mind2_SL must be > 0. Current: " + DoubleToString(Mind2_SL, 2));
        validationPassed = false;
    }

    if(Mind2_AutoCutWait < 1)
    {
        Logger(LOG_WARNING, "Mind2_AutoCutWait must be >= 1. Current: " + IntegerToString(Mind2_AutoCutWait));
        validationPassed = false;
    }
    ...
    if(Mind3_TP <= 0.0)
    {
        Logger(LOG_WARNING, "Mind3_TP must be > 0. Current: " + DoubleToString(Mind3_TP, 2));
        validationPassed = false;
    }

    if(Mind3_SL <= 0.0)
    {
        Logger(LOG_WARNING, "Mind3_SL must be > 0. Current: " + DoubleToString(Mind3_SL, 2));
        validationPassed = false;
    }

    if(Mind3_MaxConcurrent < 1)
    {
        Logger(LOG_WARNING, "Mind3_MaxConcurrent must be >= 1. Current: " + IntegerToString(Mind3_MaxConcurrent));
        validationPassed = false;
    }
    ...
    if(Mind4_TP <= 0.0)
    {
        Logger(LOG_WARNING, "Mind4_TP must be > 0. Current: " + DoubleToString(Mind4_TP, 2));
        validationPassed = false;
    }

    if(Mind4_SL <= 0.0)
    {
        Logger(LOG_WARNING, "Mind4_SL must be > 0. Current: " + DoubleToString(Mind4_SL, 2));
        validationPassed = false;
    }

    if(Mind4_AutoCutWait < 1)
    {
        Logger(LOG_WARNING, "Mind4_AutoCutWait must be >= 1. Current: " + IntegerToString(Mind4_AutoCutWait));
        validationPassed = false;
    }

    if(validationPassed)
        Logger(LOG_INFO, "InputManager: All validations passed.");
}
...
void MarketMemory_Update()
{
    ...
    if(historyCount > 0)
    {
        ...
        for(int i = 0; i < historyCount; i++)
        {
            ...
            sumMomentum   += MathAbs(observationHistory[idx].momentum);
            sumPressure   += observationHistory[idx].buyerPressure;
            sumVolatility += observationHistory[idx].candleAggressiveness;
            sumStability  += observationHistory[idx].candleStability;
            sumConfidence += observationHistory[idx].candleConfidence;
            directionSum  += observationHistory[idx].direction_M1;
        }
        ...
        historyNetDirection  = (directionSum > 0) ? 1 : (directionSum < 0) ? -1 : 0;
    }
                             ...
                             bool memoryReady)
{
    ...
    if(stateChanged)
    {
        Logger(LOG_DETECTOR,
               ...
               " | PrevState: " + EnumToString(previousMarketState) +
               " | Conf: "      + DoubleToString(detectorConfidence, 1) + "%");
    ...
    else
    {
        ...
        Logger(LOG_DETECTOR,
               ...
               " Ch="   + DoubleToString(scoreChop, 3) +
               " Vo="   + DoubleToString(scoreVol, 3)  +
               " | Bars=" + IntegerToString(stateConfirmationBars));
    }
...
void AutoCut()
{
    ...
    datetime elapsedSeconds = TimeCurrent() - autoExitStart;
    if(TimeCurrent() >= autoExitTime)
    {
        TradeManager_CloseByComment(autoExitMind);
        Logger(LOG_AUTOCUT, "AutoCut executed | Mind: " + autoExitMind +
               " | Ticket: " + autoExitMind +
               " | Elapsed: " + IntegerToString((int)elapsedSeconds) + "s" +
               " | Reason: Wait timeout");
...
// Trades only execute if FindBuyOpportunity() returns TRUE.
...
void Mind1()
{
    ...
    // Gate: Check if buy opportunity exists
    if(!FindBuyOpportunity())
    {
        Logger(LOG_TRADE, "Mind1 SKIPPED | Reason: Strategy Not Yet Defined | Placeholder: FindBuyOpportunity()");
        return;
    }

    double ask   = SymbolInfoDouble(TradingSymbol, SYMBOL_ASK);
    double tp    = ask + DollarToPoints(Mind1_TP, LotSize);
    double sl    = ask - DollarToPoints(Mind1_SL, LotSize);

    trade.Buy(LotSize, TradingSymbol, ask, sl, tp, "Mind1_StrongUptrend");
    Logger(LOG_TRADE, "Mind1 BUY | Ask: " + DoubleToString(ask, 5) +
           " | TP: " + DoubleToString(tp, 5) +
           " | SL: " + DoubleToString(sl, 5) +
           " | TPDollars: " + DoubleToString(Mind1_TP, 2) +
           " | SLDollars: " + DoubleToString(Mind1_SL, 2));
}
...
void Mind2()
{
    ...
    if(!FindBuyOpportunity())
    {
        Logger(LOG_TRADE, "Mind2 SKIPPED | Reason: Strategy Not Yet Defined | Placeholder: FindBuyOpportunity()");
        return;
    }

    double ask   = SymbolInfoDouble(TradingSymbol, SYMBOL_ASK);
    double tp    = ask + DollarToPoints(Mind2_TP, LotSize);
    double sl    = ask - DollarToPoints(Mind2_SL, LotSize);
    ...
    Logger(LOG_TRADE, "Mind2 opened | Ask: " + DoubleToString(ask, 5) +
           " | TP: " + DoubleToString(tp, 5) +
           " | SL: " + DoubleToString(sl, 5) +
           " | TPDollars: " + DoubleToString(Mind2_TP, 2) +
           " | SLDollars: " + DoubleToString(Mind2_SL, 2) +
...
// Strategy preserved exactly as defined.
// Trades only execute if FindSellOpportunity() returns TRUE.
...
void Mind3()
{
    ...
    // Gate: Check if sell opportunity exists
    if(!FindSellOpportunity())
    {
        Logger(LOG_TRADE, "Mind3 SKIPPED | Reason: Strategy Not Yet Defined | Placeholder: FindSellOpportunity()");
        return;
    }

    double bid   = SymbolInfoDouble(TradingSymbol, SYMBOL_BID);
    double tp    = bid - DollarToPoints(Mind3_TP, LotSize);
    double sl    = bid + DollarToPoints(Mind3_SL, LotSize);

    trade.Sell(LotSize, TradingSymbol, bid, sl, tp, "Mind3_StrongDowntrend");
    Logger(LOG_TRADE, "Mind3 SELL | Bid: " + DoubleToString(bid, 5) +
           " | TP: " + DoubleToString(tp, 5) +
           " | SL: " + DoubleToString(sl, 5) +
           " | TPDollars: " + DoubleToString(Mind3_TP, 2) +
           " | SLDollars: " + DoubleToString(Mind3_SL, 2));
}
...
// Strategy preserved exactly as defined.
// Trades only execute if FindOppositeTrade() returns TRUE.
...
void Mind4()
{
    ...
    // Gate: Check if opposite trade opportunity exists
    if(!FindOppositeTrade())
    {
        Logger(LOG_TRADE, "Mind4 SKIPPED | Reason: Strategy Not Yet Defined | Placeholder: FindOppositeTrade()");
        return;
    }

    double bid   = SymbolInfoDouble(TradingSymbol, SYMBOL_BID);
    double tp    = bid - DollarToPoints(Mind4_TP, LotSize);
    double sl    = bid + DollarToPoints(Mind4_SL, LotSize);
    ...
    Logger(LOG_TRADE, "Mind4 opened | Bid: " + DoubleToString(bid, 5) +
           " | TP: " + DoubleToString(tp, 5) +
           " | SL: " + DoubleToString(sl, 5) +
           " | TPDollars: " + DoubleToString(Mind4_TP, 2) +
           " | SLDollars: " + DoubleToString(Mind4_SL, 2) +
...
void OnTick()
{
    ...
    if(LogIntervalSec > 0 && currentTime % LogIntervalSec == 0)
        Logger(LOG_INFO, "Status | State: " + EnumToString(currentMarketState) +
               ...
               " | AvgMom: "   + DoubleToString(historyAvgMomentum, 5) +
               " | AvgConf: "  + DoubleToString(historyAvgConfidence, 3) +
               " | NetDir: "   + IntegerToString(historyNetDirection));
}
